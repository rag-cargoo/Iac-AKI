...
## 배경
- `labs/01-lab-docker-swarm/src/run/common/setup_env.sh`와 `swarm.py`가 첫 번째 매니저만 대상으로 동작하여 HA 구성이 불가능.
...
## 세부 작업
1. **Environment 스크립트**
  - `labs/01-lab-docker-swarm/src/run/common/setup_env.sh`에서 `manager_private_ips` 전체를 `MANAGER_PRIVATE_IPS`로 export.
   - Docker 컨텍스트 검증 로직을 모든 매니저와 워커 IP 목록과 비교하도록 수정.
2. **Ansible 인벤토리 플러그인**
  - `labs/01-lab-docker-swarm/src/ansible/inventory_plugins/swarm.py`가 매니저 리스트를 순회하여 `swarm_manager` 그룹에 모두 추가하도록 변경.
...
5. **런북 업데이트**
  - `labs/01-lab-docker-swarm/src/run/common` 스크립트와 도커 컨텍스트 사용법 문서(`AGENTS.md`, `docs/INFRA_SERVICE_STRUCTURE.md`) 갱신.
