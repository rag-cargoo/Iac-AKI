...
## 체크 리스트
- `make 01-lab-docker-swarm-setup_env` 또는 실습 디렉터리에서 `make setup_env`를 다시 실행해 `labs/01-lab-docker-swarm/src/run/common/setup_env.sh`가 Docker 전용 SSH 블록을 갱신했는지 확인합니다.
...
`labs/01-lab-docker-swarm/src/run/common/connect_service_tunnel.sh`는 서비스가 위치한 노드를 자동으로 찾아 SSH 터널을 엽니다. fingerprint를 갱신한 뒤에는 `make 01-lab-docker-swarm-tunnel`이나 관련 스크립트를 다시 실행해 새 호스트 키를 반영해야 합니다.
