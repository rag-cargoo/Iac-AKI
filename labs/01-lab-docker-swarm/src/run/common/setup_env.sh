...
TERRAFORM_DIR="$PROJECT_ROOT/src/terraform/envs/$TERRAFORM_ENVIRONMENT"
...
SSH_USER="ubuntu"
MANAGED_MARKER="labs/01-lab-docker-swarm/src/run/common/setup_env.sh"
SSH_CONFIG_OK=false
...
remove_host_block() {
  ...
  local tmp
  tmp=$(mktemp)
  awk -v marker="$host_name" '
    $0 ~ "^# Managed by .*: " marker "$" {skip=1; next}
