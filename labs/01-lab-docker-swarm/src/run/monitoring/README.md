...
이 문서는 Docker Swarm 클러스터 위에 모니터링 스택을 올릴 때 필요한 **실행 단계만** 정리합니다. 모든 명령은 `labs/01-lab-docker-swarm` 디렉터리에서 실행한다고 가정합니다.

---

## 0. 준비: Swarm 환경 변수 로드
```bash
source src/run/common/setup_env.sh
```
...
```bash
docker stack deploy -c src/run/stacks/monitoring/stack.yml monitoring
```
...
### 참고
- `src/run/stacks/monitoring/stack.yml`이 서비스 정의를, `src/run/monitoring/README.md`가 실행 단계를 담당합니다.
- 동일한 패턴으로 다른 스택도 `src/run/<service>/` 아래에 정리할 수 있습니다.
- Grafana 대시보드 JSON은 `src/run/monitoring/dashboards/` 디렉터리에 보관해 두면 Terraform을 내려도 손쉽게 재사용할 수 있습니다.
